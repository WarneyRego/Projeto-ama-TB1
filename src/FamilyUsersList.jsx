...
import './estilização/FamilyUsersList.css'
...
const FamilyUsersList = () => {
  ...
  return (
    <>
    <Navbar/>
    <nav>   <h1>Usuários Familiares</h1></nav>
      ...
      <ul className="lista">
        {familyUsers.map(user => (
          <li className="user" key={user.id} >
            {user.nome} - {user.ra} 
            <button onClick={() => handleUserClick(user.id)} className="Documents-btn">Visualizar dados do usuário <span class="folderContainer">
                  <svg
                    class="fileBack"
                    width="146"
                    height="113"
